...
use crate::stable;
use crate::{
  audit::{audit_event, set_audit_log_path},
  types::*,
};
...
impl ServerApp {
    ...
    while let Some(event) = self.node.next_event() {
      match event {
        ...
        Event::PaymentReceived {
          ...
          audit_event(
            "PAYMENT_RECEIVED",
            json!({"amount_msat": amount_msat, "payment_hash": format!("{}", payment_hash)}),
          );
        ...
        Event::ChannelClosed {
          ...
          audit_event(
            "CHANNEL_CLOSED",
            json!({"channel_id": format!("{}", channel_id), "reason": format!("{:?}", reason)}),
          );
        ...
        Ok(addr) => match addr.require_network(Network::Bitcoin) {
          Ok(valid_addr) => {
            match self
              .node
              .onchain_payment()
              .send_to_address(&valid_addr, amount, None)
            {
              Ok(txid) => {
  ...
  pub fn show_node_info_section(&mut self, ui: &mut egui::Ui) {
    ui.group(|ui| {
      ...
      ui.label(format!(
        "Listening on: {}",
        self.node.listening_addresses().unwrap()[0]
      ));
    });
  }
  ...
  pub fn update_channel_info(&mut self) -> String {
    ...
    for (i, channel) in channels.iter().enumerate() {
      let is_stable = self
        .stable_channels
        .iter()
        .any(|sc| sc.channel_id == channel.channel_id);
  ...
  pub fn designate_stable_channel(&mut self) {
    ...
    let amount = match self.stable_channel_amount.parse::<f64>() {
      ...
      Err(_) => {
        self.status_message = "Invalid amount format".to_string();
        audit_event(
          "STABLE_DESIGNATE_AMOUNT_INVALID",
    ...
    for channel in self.node.list_channels() {
      if channel.channel_id.to_string() == channel_id_str {
        ...
        audit_event(
          "STABLE_DESIGNATED",
          json!({"channel_id": channel_id_str, "target_usd": amount}),
        );
    ...
    audit_event(
      "STABLE_DESIGNATE_CHANNEL_NOT_FOUND",
  ...
  pub fn save_stable_channels(&mut self) {
    let entries: Vec<StableChannelEntry> = self
      .stable_channels
      .iter()
      .map(|sc| StableChannelEntry {
  ...
  pub fn load_stable_channels(&mut self) {
    ...
    match fs::read_to_string(&file_path) {
      Ok(contents) => match serde_json::from_str::<Vec<StableChannelEntry>>(&contents) {
        Ok(entries) => {
