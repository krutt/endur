...
impl UserApp {
  pub fn new() -> Result<Self, String> {
    ...
    let lsp_pubkey =
      PublicKey::from_str(DEFAULT_LSP_PUBKEY).map_err(|e| format!("Invalid LSP pubkey: {}", e))?;
    ...
    builder
      .set_listening_addresses(vec![format!("127.0.0.1:{}", USER_PORT).parse().unwrap()])
      .unwrap();
  ...
  fn show_waiting_for_payment_screen(&mut self, ctx: &egui::Context) {
    egui::CentralPanel::default().show(ctx, |ui| {
      ...
      ui.vertical_centered(|ui| {
        ui.heading(
          egui::RichText::new(
            "You are sending yourself bitcoin to your Stable Channels Wallet to make it stable.",
          )
          .size(16.0)
          .strong()
  ...
  fn show_onboarding_screen(&mut self, ctx: &egui::Context) {
    egui::CentralPanel::default().show(ctx, |ui| {
      ui.vertical_centered(|ui| {
        ...
        ui.heading(egui::RichText::new("Step 1: Tap Stabilize ⚡").color(egui::Color32::WHITE));
        // ui.label(
        //     egui::RichText::new("One tap to start.")
        //         .color(egui::Color32::GRAY),
        // );
        ui.add_space(20.0);
        ui.heading(egui::RichText::new("Step 2: Fund your wallet 💸").color(egui::Color32::WHITE));
        ui.label(
          egui::RichText::new("Send yourself bitcoin over Lightning").color(egui::Color32::GRAY),
        );
        ui.add_space(20.0);
        ui.heading(
          egui::RichText::new("Step 3: Enjoy your stabilized BTC 🔧").color(egui::Color32::WHITE),
        );
        ui.label(
              ...
              if ui.button("Withdraw all to address").clicked() {
                match ldk_node::bitcoin::Address::from_str(&self.on_chain_address) {
                  Ok(addr) => match addr.require_network(ldk_node::bitcoin::Network::Bitcoin) {
                    Ok(valid_addr) => match self.node.onchain_payment().send_all_to_address(
                      &valid_addr,
                      false,
                      None,
                    ) {
...
pub fn run() {
  ...
  let native_options = eframe::NativeOptions {
    viewport: eframe::egui::ViewportBuilder::default().with_inner_size([460.0, 700.0]),
    ..Default::default()
  };
